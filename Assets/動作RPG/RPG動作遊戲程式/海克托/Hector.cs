...
public class Hector : MonoBehaviour
{
    ...
    // 定義事件，當 Boss 使用技能時觸發
    public event Action<HectorSkill> OnSkillUsed;
    ...
    private void Update()
    {
        ...
        if (!stop)
        {
            ...
            if (IntervalTime > skillTime + 0.2f)
            {
                UseRandomSkill();
                IntervalTime = 0;
            }
    ...
    public void UseRandomSkill()
    {
        ...
        // 從技能清單中隨機選擇一個技能
        HectorSkill randomSkill = (HectorSkill)random.Next(Enum.GetNames(typeof(HectorSkill)).Length);
        ...
        switch (randomSkill)
        {
            case HectorSkill.Skill1:
                if (!isSkill1CanUse)
                {
                    canUseSkill = false;
                }
                break;
            case HectorSkill.Skill2:
                if (!isSkill2CanUse)
                {
                    canUseSkill = false;
                }
                break;
            case HectorSkill.Skill3:
                if (!isSkill3CanUse)
                {
                    canUseSkill = false;
                }
                break;
            case HectorSkill.Skill4:
                if (!isSkill4CanUse)
                {
                    canUseSkill = false;
                }
