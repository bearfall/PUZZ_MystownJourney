...
public class GeusSmall : MonoBehaviour
{
    public Transform attakTarget;

    public enum GeusSmallSkill
    {
        Skill1,
        Skill2,
        Skill3,
    ...
    // 定義事件，當 Boss 使用技能時觸發
    public event Action<GeusSmallSkill> OnSkillUsed;
    ...
    public void UseRandomSkill()
    {
        ...
        // 從技能清單中隨機選擇一個技能
        GeusSmallSkill randomSkill = (GeusSmallSkill)random.Next(Enum.GetNames(typeof(GeusSmallSkill)).Length);
        ...
        switch (randomSkill)
        {
            case GeusSmallSkill.Skill1:
                if (!isSkill1CanUse)
                {
                    canUseSkill = false;
                }
                break;
            case GeusSmallSkill.Skill2:
                if (!isSkill2CanUse)
                {
                    canUseSkill = false;
                }
                break;
            case GeusSmallSkill.Skill3:
                if (!isSkill3CanUse)
                {
                    canUseSkill = false;
                }
                break;
            case GeusSmallSkill.Skill4:
                if (!isSkill4CanUse)
                {
                    canUseSkill = false;
                }
    ...
    public void StartGeusSmallAction()
    {
        stop = false;
        IntervalTime = 0;
        skillTime = 0;
    ...
    public void StopGeusSmallAction()
    {
        stop = true;
        IntervalTime = 0;
        skillTime = 0;
